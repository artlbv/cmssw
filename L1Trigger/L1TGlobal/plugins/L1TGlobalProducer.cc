...
void L1TGlobalProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("produceMLScore", false);
...
L1TGlobalProducer::L1TGlobalProducer(const edm::ParameterSet& parSet)
    : m_muInputTag(parSet.getParameter<edm::InputTag>("MuonInputTag")),
      ...
      m_produceMLScore(parSet.getParameter<bool>("produceMLScore")) {
  ...
  if (m_produceMLScore) {
    produces<MLScoreBxCollection>("MLScore");
  }
...
*/
  ...
  std::unique_ptr<MLScoreBxCollection> uGtMLScoreRecord(nullptr);
  if (m_produceMLScore) {
    uGtMLScoreRecord = std::make_unique<MLScoreBxCollection>();
  }
  ...
  m_uGtBrd->enableMLScoreSaving(m_produceMLScore);
  ...
  for (int iBxInEvent = minEmulBxInEvent; iBxInEvent <= maxEmulBxInEvent; ++iBxInEvent) {
    ...
    if (m_produceMLScore) {
      m_uGtBrd->fillMLScore(iBxInEvent, uGtMLScoreRecord);
    }
  ...
  if (m_produceMLScore) {
    iEvent.put(std::move(uGtMLScoreRecord), "MLScore");
  }
}
