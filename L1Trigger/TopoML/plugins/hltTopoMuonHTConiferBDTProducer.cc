...
HLTTopoMuonHTConiferBDTProducer::HLTTopoMuonHTConiferBDTProducer(edm::ParameterSet const& iConfig)
    : l1tTopoScoreToken_(consumes<float>(iConfig.getParameter<edm::InputTag>("l1tTopoScore"))),
      ...
      ecalIsoMapToken_(consumes<RecoChargedCandMap>(iConfig.getParameter<edm::InputTag>("EcalPFClusterIsoMap"))),
      hcalIsoMapToken_(consumes<RecoChargedCandMap>(iConfig.getParameter<edm::InputTag>("HcalPFClusterIsoMap"))),
...
void HLTTopoMuonHTConiferBDTProducer::produce(edm::StreamID,
  ...
  for (int i = 0, n = static_cast<int>(chargedCandidatesH->size()); i < n; ++i) {
  ...
  if (bestIdx >= 0) {
    const reco::RecoChargedCandidateRef muonRef = edm::getRef(chargedCandidatesH, bestIdx);

    // Get isolations (invert to match training: iso -> 10 - iso with 10 being the default for missing)
...
void HLTTopoMuonHTConiferBDTProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("l1tTopoScore", edm::InputTag("l1tTopoBDTProducer", "score"));
