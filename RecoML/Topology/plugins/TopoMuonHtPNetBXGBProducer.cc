 ...
 *  the leading N PNetB scores among jets in the event (configurable),
 *  and the pt and isolation variables of up to N muons (configurable, 0 = no muon features).
 *
...
public:
  ...
  // 1 global feature (PFHT) + nPNetB_ PNetB scores + 4 features per muon (pt, tkIso, ecalIso, hcalIso)
...
TopoMuonHtPNetBXGBProducer::TopoMuonHtPNetBXGBProducer(edm::ParameterSet const& iConfig)
  ...
  // Feature layout: [PFHT | PNetB_0 .. PNetB_nPNetB-1 | mu0_pt, mu0_tkIso, mu0_ecalIso, mu0_hcalIso | mu1_... ]
...
void TopoMuonHtPNetBXGBProducer::produce(edm::StreamID, edm::Event& iEvent, edm::EventSetup const& setup) const {
  ...
  if (nMuons_ > 0) {
    ...
    if (muonSortByTkIso_) {
      ...
      std::sort(muonIndices.begin(), muonIndices.end(), [&](size_t a, size_t b) {
        const float isoA = trkH.isValid() ? static_cast<float>((*trkH)[edm::getRef(muonsH, a)])
                                          : std::numeric_limits<float>::max();
        const float isoB = trkH.isValid() ? static_cast<float>((*trkH)[edm::getRef(muonsH, b)])
                                          : std::numeric_limits<float>::max();
  ...
  if (debug_) {
    ...
    ss << "TopoMuonHtPNetBXGBProducer:"
       << " nPNetB(found)=" << pnetScores.size()
       << " nMuons(found)=" << (nMuons_ > 0 ? 0 : 0)  // placeholder; see note below
       << "\n Features: ";
...
void TopoMuonHtPNetBXGBProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<unsigned int>("nMuons", 1)
      ->setComment("number of muons used as input features; 0 = no muon features (b-jet + HT only model)");
