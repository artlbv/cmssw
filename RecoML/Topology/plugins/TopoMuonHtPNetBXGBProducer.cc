...
class TopoMuonHtPNetBXGBProducer : public edm::global::EDProducer<> {
 ...
 private:
  ...
  /* Config */
  const unsigned int
      nMuons_;  // number of muons used as input features (0 = no muon features)
  const unsigned int
      nPNetB_;  // number of leading PNetB scores used as input features
...
TopoMuonHtPNetBXGBProducer::TopoMuonHtPNetBXGBProducer(
  ...
  if (debug_) {
    std::cout << "Loading XGBoost model from " << modelPath.fullPath() << "\n"
              ...
              << " nTreeLimit=" << nTreeLimit_ << std::endl;
  }
...
void TopoMuonHtPNetBXGBProducer::produce(edm::StreamID, edm::Event& iEvent,
  ...
  if (const auto& h = iEvent.getHandle(pnetToken_); h.isValid()) {
    ...
    for (auto const& tag : *h) pnetScores.push_back(tag.second);
  ...
  const unsigned int nPNetBFill =
      std::min(pnetScores.size(), static_cast<size_t>(nPNetB_));
  ...
  if (nMuons_ > 0) {
    ...
    if (muonSortByTkIso_) {
      ...
      std::sort(muonIndices.begin(), muonIndices.end(),
                [&](size_t a, size_t b) {
                  const float isoA =
                      trkH.isValid()
  ...
  if (debug_) {
    ...
    ss << "TopoMuonHtPNetBXGBProducer:"
       << " nPNetB(found)=" << pnetScores.size() << " nMuons(found)="
       << (nMuons_ > 0 ? 0 : 0)  // placeholder; see note below
       << "\n Features: ";
...
void TopoMuonHtPNetBXGBProducer::fillDescriptions(
  ...
  desc.add<edm::InputTag>("ChargedCandidates",
                          edm::InputTag("hltIterL3MuonCandidates"));
  desc.add<edm::InputTag>("EcalPFClusterIsoMap",
                          edm::InputTag("hltMuonEcalMFPFClusterIsoForMuons"));
  desc.add<edm::InputTag>("HcalPFClusterIsoMap",
                          edm::InputTag("hltMuonHcalRegPFClusterIsoForMuons"));
  desc.add<edm::InputTag>("TrackIsoMap",
                          edm::InputTag("hltMuonTkRelIsolationCut0p3Map",
